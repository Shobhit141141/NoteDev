...
const defineThemes = (monaco:any) => {
  ...
  // Define the "lc" theme based on the provided screenshot
...
const MonacoEditor = () => {
  ...
  const [theme, setTheme] = useState<string>('lc'); // Set default theme to 'lc'
  const [language, setLanguage] = useState<string>('javascript');
  const [fontSize, setFontSize] = useState<number>(14); // Set default font size to 14

  const themes = [
    'vs-dark', 'hc-black', 'monokai', 'dracula', 'lc'
  ];
  ...
  const handleEditorMount: OnMount = (editor, monaco) => {
    ...
    monaco.editor.setTheme('lc'); // Apply the 'lc' theme immediately
  };
